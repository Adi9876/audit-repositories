...
contract PuppyRaffle is ERC721, Ownable {
    ...
    function getActivePlayerIndex(address player) external view returns (uint256) {
        ...
        // q what if the player is at index 0: it returns 0 right
        // but for not active then also the first player (index 0) is selected. ??
        // player at index 0 will always be inactive ??
        return 0;
    }
    ...
    function selectWinner() external {
        ...
        // @audit - undeflow/overflow issue
        // @audit - unsafe casting of unit256 to uint64
        totalFees = totalFees + uint64(fee);

        uint256 tokenId = totalSupply();

        // We use a different RNG calculate from the winnerIndex to determine rarity
        uint256 rarity = uint256(keccak256(abi.encodePacked(msg.sender, block.difficulty))) % 100; // e - winnerIndex not include ??
